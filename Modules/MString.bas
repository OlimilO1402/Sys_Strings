...
Private Type TLong4
    ...
    Hi2 As Long
End Type
Private Type TCurrency2
    Lo As Currency
    Hi As Currency
End Type
Private Type TDecimal
    Value As Variant
End Type
...
Public Function FormatMByte(CountElements As Long, vt As VbVarType) As String
    Dim bpe As Long
    Select Case vt
    Case VbVarType.vbByte:                                  bpe = 1
    Case VbVarType.vbInteger, VbVarType.vbString:           bpe = 2 ' 2 bytes per character
    Case VbVarType.vbLong, VbVarType.vbSingle:              bpe = 4
    Case VbVarType.vbCurrency, VbVarType.vbDouble, VbVarType.vbDate:  bpe = 8
    Case VbVarType.vbDecimal, VbVarType.vbVariant:          bpe = 16
    End Select
...
Public Function Bin_ToStr(ByVal Value, Optional AddHead As Boolean = True) As String
    Dim s As String: s = IIf(AddHead, "&B", "")
...
#End If
    Case VbVarType.vbSingle
        Dim ts As TSingle:  ts.Value = CSng(Value)
        Dim tl As TLong:     LSet tl = ts
                                   s = s & Bin32(tl.Value)
    Case VbVarType.vbDouble
        Dim td As TDouble:  td.Value = CDbl(Value)
        Dim TC As TCurrency: LSet TC = td
                                   s = s & Bin64(TC.Value)
    Case VbVarType.vbCurrency:     s = s & Bin64(CCur(Value))
    Case VbVarType.vbDecimal:
        'Dim tdc As TDecimal: tdc.Value = CDec(Value)
        'Dim tl4 As TLong4:    LSet tl4 = tdc
        '                             s = s & BinLng_ToStr(tl4.Lo1) & BinLng_ToStr(tl4.Lo2) & BinLng_ToStr(tl4.Hi1) & BinLng_ToStr(tl4.Hi2)
    Case Else
        If (vt And VbVarType.vbArray) = VbVarType.vbArray Then
            Dim i As Long
            For i = LBound(Value) To UBound(Value)
                s = s & Bin_ToStr(Value(i), False)
            Next
        End If
    End Select
    Bin_ToStr = s
...
Function PadLeft(ByVal this As String, ByVal totalWidth As Long, Optional ByVal paddingChar As String) As String
...
Public Function PadCentered(ByVal this As String, ByVal totalWidth As Long, Optional ByVal paddingChar As String) As String
...
Function PadRight(ByVal this As String, ByVal totalWidth As Long, Optional ByVal paddingChar As String) As String
...
Public Function AddQuotesIfNot(ByVal s As String) As String
    's = Trim(s)
    If Len(s) = 0 Then AddQuotesIfNot = """""": Exit Function
    If Left(s, 1) <> """" Then s = """" & s
    If Right(s, 1) <> """" Then s = s & """"
End Function

Public Function RemoveQuotes(ByVal s As String) As String
    s = Trim(s)
    If Len(s) = 0 Then Exit Function
